...
class ContagionGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final dotPaint = Paint()..color = const Color(0xFF1E2330);
  ...
  void _drawEdge(Canvas canvas, Offset start, Offset end, String tier) {
    ...
    final controlPoint1 = Offset(start.dx, start.dy + (end.dy - start.dy) * 0.5);
    ...
    Color color = AppColors.borderDefault.withAlpha(128);

    if (tier == "T1") {
      strokeWidth = 1.5;
      color = AppColors.accentCrimson.withAlpha(77); // Root to T1
    } else if (tier == "T2") {
      strokeWidth = 1.0;
      color = AppColors.accentAmber.withAlpha(77); // T1 to T2
    } else {
      strokeWidth = 0.75;
      color = AppColors.accentBlue.withAlpha(51); // T2 to T3
    }
  ...
  void _drawTravelingDot(Canvas canvas, Offset start, Offset end, String tier) {
    ...
    if (tangent != null) {
      Color dotColor;
      if (tier == "T1") dotColor = AppColors.accentCrimson;
      else if (tier == "T2") dotColor = AppColors.accentAmber;
      else dotColor = AppColors.accentBlue;
      ...
      // Outer glow
      canvas.drawCircle(tangent.position, 5.0, Paint()..color = dotColor.withAlpha(51));
    }
  }
  ...
  void _drawNode(Canvas canvas, Offset pos, ContagionNode node) {
    ...
    if (node.tier == "ROOT") {
      // GLOW
      canvas.drawCircle(pos, 50, Paint()..color = AppColors.accentCrimson.withAlpha(38));
      ...
      // GLOW
      canvas.drawCircle(pos, 38, Paint()..color = AppColors.accentAmber.withAlpha(26));
      ...
      canvas.drawCircle(pos, 10, Paint()..color = AppColors.bgTertiary);
      canvas.drawCircle(
        pos, 
        10, 
        Paint()
      ...
      canvas.drawCircle(pos, 3, Paint()..color = AppColors.accentBlue);
    ...
    final textPainter = TextPainter(
      text: TextSpan(
        text: node.label.length > 18 ? "${node.label.substring(0, 15)}..." : node.label,
        style: TextStyle(
          fontFamily: 'IBM Plex Mono',
          fontSize: 9,
          color: node.tier == "ROOT" ? AppColors.accentCrimson : AppColors.textSecondary,
        ),
      ),
  ...
  // Helper for tap detection (renamed from hitTest to avoid CustomPainter override conflict)
